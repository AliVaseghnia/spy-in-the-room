...
const { applyCardAction, NotFoundError } = require('../../../server/game-service.js');
...
async function handleRequest(request, response, options = {}) {
  ...
  try {
    ...
    const input = validateCardRequest(await parseJsonBody(request));
    const session = await getSessionFromRequest(request, store, config, now);
    if (!session) throw new NotFoundError();
    const responseBody = await applyCardAction({
      store,
